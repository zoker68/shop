...
<nav class="bg-secondary lg:block">
    <div class="container flex items-center justify-between">
        ...
        <!-- All categories -->
        <div class="bg-secondary w-[210px] relative hidden lg:block group-hbr mr-8">
            <div class="py-6 px-4 flex items-center justify-center w-full">
            ...
            <div
                class="absolute top-full left-0 w-[250px] bg-white shadow-md z-10 mt-2.5 opacity-0 invisible group-hvr-hover">
        ...
        <div class="flex items-center">
            ...
            <div class="relative group">
                <a href="{{ route('account.profile.dashboard') }}" class="relative block text-center ml-5">
                    ...
                    <span class="text-white text-[11px] leading-[10px]">{{ __('shop::layout.header.navbar.account') }}</span>
                </a>

                <div
                    class="absolute top-full right-[1px] bg-white z-20 rounded-md p-5 w-[245px] shadow-xl mt-3.5 group-hover:mt-[5px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                    @guest()
                        <div>
                    ...
                    @auth()
                        <div class="flex flex-col gap-1.5">
                            <a href="{{ route('account.profile.dashboard') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-secondary transition duration-200">
                                ...
                                {{ __('shop::layout.header.navbar.account-menu.account') }}
                            </a>
                            <a href="{{ route('account.orders.index') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-secondary transition duration-200">
                                ...
                                {{ __('shop::layout.header.navbar.account-menu.orders') }}
                            </a>
                            <a href="{{ route('account.wishlist') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-secondary transition duration-200">
                                ...
                                {{ __('shop::layout.header.navbar.account-menu.wishlist') }}
                            </a>
                            <a href="{{ route('cart') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-secondary transition duration-200">
                                ...
                                {{ __('shop::layout.header.navbar.account-menu.cart') }}
                            </a>
                            <form action="{{ route('logout') }}" method="POST">
                                ...
                                <button type="submit"
                                    ...
                                    {{ __('shop::layout.header.navbar.account-menu.logout') }}
                                </button>
                            </form>
                        </div>
                    @endauth
