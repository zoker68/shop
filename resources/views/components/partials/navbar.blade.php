    ...
    <div class="container flex items-center justify-between">
        ...
        <div class="bg-secondary py-1.5 w-[210px] relative lg:block group-hbr mr-8">
            <div class="py-2.5 px-4 flex items-center justify-center w-full">
                ...
                <span class="text-white text-base">{{ __('zoker.shop::layout.header.navbar.all_categories') }}</span>
            </div>

            <div
                class="absolute top-full left-0 w-full py-4 bg-white shadow-md z-10 mt-2.5 opacity-0 invisible group-hvr-hover">
                @foreach($categories as $category)
                    <x-zoker.shop::partials.navbar.primary-link :category="$category"/>
                @endforeach
            </div>
        </div>
        ...
        <div class="flex items-center">
            ...
            <div class="relative group">
                <a href="{{ route('account.profile.dashboard') }}" class="relative block text-center ml-5">
                    ...
                    <span class="text-white text-[11px] leading-[10px]">{{ __('zoker.shop::layout.header.navbar.account') }}</span>
                </a>
                    ...
                    @guest()
                        <div>
                            <div class="flex justify-between">
                                <a href="#"
                                   class="min-w-[85px] rounded-[3px] py-1 px-[15px] border border-primary bg-primary text-white inline-block text-center text-sm font-medium hover:bg-transparent hover:text-primary transition duration-300">{{ __('zoker.shop::layout.header.navbar.registration') }}</a>
                                <a href="{{ route('login') }}"
                                   class="min-w-[85px] rounded-[3px] py-1 px-2.5 border border-primary hover:bg-primary bg-white hover:text-white inline-block text-center text-sm font-medium text-primary transition duration-300">
                                    {{ __('zoker.shop::layout.header.navbar.login') }}
                                </a>
                            </div>
                        </div>
                    @endguest
                        ...
                        <div class="pt-2.5">
                            <a href="{{ route('account.profile.dashboard') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-primary transition duration-200">
                                ...
                                {{ __('zoker.shop::layout.header.navbar.account-menu.account') }}
                            </a>
                            <a href="{{ route('account.orders.index') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-primary transition duration-200">
                                ...
                                {{ __('zoker.shop::layout.header.navbar.account-menu.orders') }}
                            </a>
                            <a href="{{ route('account.wishlist') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-primary transition duration-200">
                                ...
                                {{ __('zoker.shop::layout.header.navbar.account-menu.wishlist') }}
                            </a>
                            <a href="{{ route('cart') }}"
                               class="flex items-center relative w-full mt-[7px] text-[15px] pl-8 text-[#464545] hover:text-primary transition duration-200">
                                ...
                                {{ __('zoker.shop::layout.header.navbar.account-menu.cart') }}
                            </a>
                            <form action="{{ route('logout') }}" method="POST">
                                ...
                                <button type="submit"
                                    ...
                                    {{ __('zoker.shop::layout.header.navbar.account-menu.logout') }}
                                </button>
                            </form>
                        </div>
                    @endauth
