...
<div class="pt-4">
    ...
    <div>
        @if (count($addresses) > 0)
            <div class="mt-4">
                <x-form.label for="{{ $addressType }}_select">{{ __('shop::checkout.address.select.label') }}</x-form.label>
        ...
        @if (!(isset($address[$addressType]['id']) && $address[$addressType]['id'] > 0))
            <div class="sm:flex md:block lg:flex gap-6 mt-6">
                <div class="w-full @if(count($regions[$addressType]) > 0)sm:w-1/2 md:w-full lg:w-1/2 @endif">
                    <x-form.label for="{{ $addressType }}_country" required="true">{{ __('shop::checkout.address.country.label') }}</x-form.label>
                ...
                @if(count($regions[$addressType]) > 0)
                    <div class="w-full sm:w-1/2 md:w-full lg:w-1/2 mt-6 lg:mt-0">
                        <x-form.label for="{{ $addressType }}_region">{{ __('shop::checkout.address.region.label') }}</x-form.label>
            ...
            <div class="sm:flex md:block lg:flex gap-6 mt-6">
                <div class="w-full sm:w-1/2 md:w-full lg:w-1/2">
                    <x-form.label for="{{ $addressType }}_town_city" required="true">{{ __('shop::checkout.address.city') }}</x-form.label>
                ...
                <div class="w-full sm:w-1/2 md:w-full lg:w-1/2 mt-6 lg:mt-0">
                    <x-form.label for="{{ $addressType }}_zip_code" required="true">{{ __('shop::checkout.address.zip') }}</x-form.label>
            ...
            <div class="mt-4">
                <x-form.label for="{{ $addressType }}_street_addr" required="true">{{ __('shop::checkout.address.address') }}</x-form.label>
