<x-zoker.shop::layouts.app>
    <x-zoker.shop::partials.breadcrumbs :data="[['title' => __('zoker.shop::layout.account.breadcrumbs.account'), 'url' => route('account.profile.dashboard')]]"/>
            ...
            <div @click.outside="isOpen=false">
                <div class="box_shadow px-4 py-2 flex gap-5 items-center">
                    ...
                    <div class="acprof_cont">
                        <p>{{ __('zoker.shop::layout.account.welcome') }}</p>
                ...
                <div :class="isOpen ? 'opacity-100 visible top-20' : 'opacity-0 invisible top-0'"
                    ...
                    <div class="border-b pb-3">
                        <x-partials.account.nav-top :active="request()->routeIs('account.profile.*')" href="{{ route('account.profile.dashboard') }}">
                            <span>
                            ...
                            {{ __('zoker.shop::layout.account.dashboard') }}
                        </x-partials.account.nav-top>
                        <x-partials.account.nav-second route="account.profile.index">{{ __('zoker.shop::layout.account.profile') }}</x-partials.account.nav-second>
                        <x-partials.account.nav-second route="account.profile.address.index">{{ __('zoker.shop::layout.account.address') }}</x-partials.account.nav-second>
                        <x-partials.account.nav-second route="account.profile.password.edit">{{ __('zoker.shop::layout.account.password') }}</x-partials.account.nav-second>
                    </div>
                    <div class="border-b mt-3 pb-3">
                        <x-partials.account.nav-top :active="request()->routeIs('account.orders.*')" href="{{ route('account.orders.index') }}">
                            ...
                            {{ __('zoker.shop::layout.account.orders') }}
                        </x-partials.account.nav-top>
                    </div>
                    <div class="border-b mt-3 pb-3">
                        <x-partials.account.nav-top :active="request()->routeIs(['account.wishlist'])" href="{{ route('account.wishlist') }}">
                            <span class="group-hover:text-primary">
                            ...
                            {{ __('zoker.shop::layout.account.wishlist') }}
                        </x-partials.account.nav-top>
                    </div>
                    <div class="mt-4">
                        <form action="{{ route('logout') }}" method="POST">
                            ...
                            <button type="submit"
                               class="flex gap-2 items-center text-[18px] font-medium hover:text-primary group">
                                ...
                                {{ __('zoker.shop::layout.account.logout') }}
                            </button>
                        </form>
                    </div>
                </div>
