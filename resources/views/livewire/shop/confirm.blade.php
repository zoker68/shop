        ...
        <div class="container grid grid-cols-12 gap-6 pb-14">
            <div class="col-span-12 md:col-span-6 lg:col-span-8">
                ...
                @foreach(['shipping', 'billing'] as $addressType)
                    <h4 class="bg-secondary text-white px-3 py-2">{{ __('zoker68.shop::checkout.' . $addressType . '_address.title') }}</h4>
                    <div class="grid grid-cols-3 my-4 border">
                        <div class="px-3 py-2">{{  __('zoker68.shop::checkout.address.country.label') }} </div>
                        <div class="px-3 py-2 col-span-2">{{ $cart[$addressType . 'Address']->country->name  }}</div>
                        @if($cart[$addressType . 'Address']->region)
                            <div class="px-3 py-2">{{ __('zoker68.shop::checkout.address.region.label') }}:</div>
                            <div class="px-3 py-2 col-span-2">{{ $cart[$addressType . 'Address']->region->name }}</div>
                        @endif
                        <div class="px-3 py-2">{{ __('zoker68.shop::checkout.address.city') }}:</div>
                        <div class="px-3 py-2 col-span-2">{{ $cart[$addressType . 'Address']->city }}</div>
                        <div class="px-3 py-2">{{ __('zoker68.shop::checkout.address.zip') }}:</div>
                        <div class="px-3 py-2 col-span-2">{{ $cart[$addressType . 'Address']->zip }}</div>
                        <div class="px-3 py-2">{{ __('zoker68.shop::checkout.address.address') }}:</div>
            ...
            <div class="col-span-12 md:col-span-6 lg:col-span-4">
                ...
                <div class="gap-3 items-center mt-4">
                    ...
                    <x-form.label for="agreeToTerms" class="text-sm cursor-pointer inline-flex items-center">
                        {{ __('zoker68.shop::checkout.agree_with') }} <a href="terms-condition.html"
                                                           class="text-primary">{{ __('zoker68.shop::checkout.agree_with_link') }}</a>
                    </x-form.label>
