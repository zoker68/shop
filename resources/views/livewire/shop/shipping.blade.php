<div class="container grid grid-cols-12 gap-6 pb-14">
    <div class="col-span-12 lg:col-span-8">
        <h4 class="bg-secondary text-white px-3 py-2">{{ __('zoker68.shop::checkout.shipping.title') }}</h4>
                ...
                <div x-show="activeTab==='shipping_method_{{ $shippingMethod->id }}'"
                    ...
                    <div>
                        <div class="text-left mt-4 pb-4">
                            ...
                            @endif
                            <p class="pt-5">{{ __('zoker68.shop::checkout.shipping.days') }} <span
                                    class="font-semibold">{{ $shippingMethod->days }}</span></p>
                            <p>{{ __('zoker68.shop::checkout.shipping.price') }} <span class="font-semibold">@money($shippingMethod->price)</span></p>
                            <x-form.button class="mt-4" wire:click="setShippingMethod({{ $shippingMethod->id }})">{{ __('zoker68.shop::checkout.shipping.submit') }}</x-form.button>
                        </div>
                    </div>
                </div>
            @endforeach
