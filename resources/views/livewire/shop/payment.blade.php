<div class="container grid grid-cols-12 gap-6 pb-14">
    <div class="col-span-12 lg:col-span-8">
        <h4 class="bg-secondary text-white px-3 py-2">{{ __('zoker.shop::checkout.payment.title') }}</h4>
                ...
                <div x-show="activeTab==='shipping_method_{{ $paymentMethod->id }}'"
                    ...
                    <div>
                        <div class="text-left mt-4 pb-4">
                            ...
                            <p>{{ __('zoker.shop::checkout.payment.fee') }} <span
                                    class="font-semibold">@money($paymentMethod->fee)</span></p>
                            @if($paymentMethod->fee_percent > 0)
                                <p>
                                    {{ __('zoker.shop::checkout.payment.fee_percent') }} <span
                                        class="font-semibold">{{ $paymentMethod->fee_percent }}%</span>
                                </p>
                            @endif
                            <x-form.button class="mt-4"
                                           wire:click="setPaymentMethod('{{ $paymentMethod->id }}')">{{ __('zoker.shop::checkout.payment.submit') }}</x-form.button>
                        </div>
                    </div>
                </div>
            @endforeach
