<div class="container pb-10">
        ...
        <div class="grid grid-cols-12 gap-6">
            <div class="col-span-12 lg:col-span-9">
                <div class="grid grid-cols-6 md:grid-cols-7 w-full">
                    ...
                    <div class="cart-header col-span-3 md:col-span-4">{{ __('cart.column.product') }}</div>
                    <div class="cart-header">{{ __('cart.column.quantity') }}</div>
                    <div class="cart-header col-span-2 md:col-span-1">{{ __('cart.column.total_price') }}</div>
                    ...
                    @foreach($cart->products as $item)
                        ...
                        <div
                            ...
                            @if(!$item->hasStock())
                                <p class="text-red-500">{{ __('cart.error.stock', ['quantity' => $item->product->stock]) }}</p>
                            @endif
                            @if(!$item->product->isAvailable())
                                <p class="text-red-500">{{ __('cart.error.available') }}</p>
                            @endif
                        </div>
    ...
    @else
        <x-partials.flash-alert type="info">{{ __('cart.error.empty') }}</x-partials.flash-alert>
    @endif
</div>
