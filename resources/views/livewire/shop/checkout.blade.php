    ...
    <form wire:submit="onShipping">
        <div class="container grid grid-cols-12 gap-6 pb-14">
            <div class="col-span-12 md:col-span-6 lg:col-span-8">
                <h4 class="bg-secondary text-white px-3 py-2">{{ __('zoker68.shop::checkout.customer.title') }}</h4>
                @guest()
                    <div>
                        <div class="mt-4">
                            <x-form.label for="email" required="true">{{ __('zoker68.shop::checkout.customer.email') }}</x-form.label>
                            <x-form.input id="email" type="email" required="true" wire:model.blur="mainData.email"/>

                            <x-form.error name="mainData.email"/>
                            {{ __('zoker68.shop::checkout.customer.have_account') }} <a href="{{ route('login') }}"
                                                                          class="text-primary">{{ __('zoker68.shop::checkout.customer.have_account_link') }}</a>
                        </div>
                        <div class="mt-4">
                            <x-form.checkbox id="asGuest" wire:model.live="asGuest"/>
                            <x-form.label for="asGuest"
                                          class="inline-flex items-center pl-3">{{ __('zoker68.shop::checkout.customer.as_guest') }}</x-form.label>
                        </div>
                        @if(!$asGuest)
                            <div class="sm:flex md:block lg:flex gap-6 mt-6">
                                <div class="w-full sm:w-1/2 md:w-full lg:w-1/2">
                                    <x-form.label for="password"
                                                  required="true">{{ __('zoker68.shop::checkout.customer.password') }}</x-form.label>
                                ...
                                <div class="w-full sm:w-1/2 md:w-full lg:w-1/2 mt-6 lg:mt-0">
                                    <x-form.label for="password_confirmation" required="true">
                                        {{ __('zoker68.shop::checkout.customer.password_confirmation') }}
                                    </x-form.label>
                        ...
                        <div class="sm:flex md:block lg:flex gap-6 mt-6">
                            <div class="w-full sm:w-1/2 md:w-full lg:w-1/2">
                                <x-form.label for="first_name"
                                              required="true">{{ __('zoker68.shop::checkout.customer.first_name') }}</x-form.label>
                            ...
                            <div class="w-full sm:w-1/2 md:w-full lg:w-1/2 mt-6 lg:mt-0">
                                <x-form.label for="last_name"
                                              required="true">{{ __('zoker68.shop::checkout.customer.last_name') }}</x-form.label>
                        ...
                        <div class="sm:flex md:block lg:flex gap-6 mt-6">
                            <div class="w-full sm:w-1/2 md:w-full lg:w-1/2">
                                <x-form.label for="phone_number"
                                              required="true">{{ __('zoker68.shop::checkout.customer.phone') }}</x-form.label>
                                <x-form.input id="phone_number" type="text" required="true"
                                              wire:model="mainData.phone"/>
                                <x-form.error name="mainData.phone"/>
                            </div>
                            <div class="w-full sm:w-1/2 md:w-full lg:w-1/2 mt-6 lg:mt-0">
                                <x-form.label for="birthday" required="true">{{ __('zoker68.shop::checkout.customer.birthday') }}</x-form.label>
                        ...
                        <div class="sm:flex md:block lg:flex gap-6 mt-6">
                            <div class="w-full sm:w-1/2 md:w-full lg:w-1/2">
                                <x-form.label for="company_name">{{ __('zoker68.shop::checkout.customer.company') }}</x-form.label>
                                <x-form.input id="company_name" type="text"
                                              wire:model="mainData.company"/>
                                <x-form.error name="mainData.company"/>
                            </div>
                            <div class="w-full sm:w-1/2 md:w-full lg:w-1/2 mt-6 lg:mt-0">
                                <x-form.label for="vat_number">{{ __('zoker68.shop::checkout.customer.vat') }}</x-form.label>
                ...
                @auth()
                    <div class="grid grid-cols-3 my-4 border">
                        <div class="px-3 py-2">{{ __('zoker68.shop::checkout.customer.first_name') }}:</div>
                        <div class="px-3 py-2 col-span-2">{{ auth()->user()->name }}</div>
                        <div class="px-3 py-2">{{ __('zoker68.shop::checkout.customer.last_name') }}:</div>
                        <div class="px-3 py-2 col-span-2">{{ auth()->user()->surname }}</div>
                        <div class="px-3 py-2">{{ __('zoker68.shop::checkout.customer.email') }}:</div>
                        <div class="px-3 py-2 col-span-2">{{ auth()->user()->email }}</div>
                        <div class="px-3 py-2">{{ __('zoker68.shop::checkout.customer.phone') }}:</div>
                        <div class="px-3 py-2 col-span-2">{{ auth()->user()->phone }}</div>
                        @if(auth()->user()->company)
                            <div class="px-3 py-2">{{ __('zoker68.shop::checkout.customer.company') }}:</div>
                            <div class="px-3 py-2 col-span-2">{{ auth()->user()->company }}</div>
                        @endif
                        @if(auth()->user()->vat)
                            <div class="px-3 py-2">{{ __('zoker68.shop::checkout.customer.vat') }}:</div>
                ...
                <x-partials.checkout.address :countries="$countries" :regions="$regions" :addresses="$addresses"
                    ...
                    {{ __('zoker68.shop::checkout.shipping_address.title') }}
                ...
                <div class="mt-4">
                    ...
                    <label for="billing_address_same" class="inline-flex items-center">
                        <span class="pl-3">{{ __('zoker68.shop::checkout.billing_address.same_as_shipping') }}</span>
                    </label>
                </div>
                ...
                @if(!$billingSameAsShipping)
                    <x-partials.checkout.address :countries="$countries" :regions="$regions" :addresses="$addresses"
                        ...
                        {{ __('zoker68.shop::checkout.billing_address.title') }}
                    </x-partials.checkout.address>
                @endif
            </div>
            ...
            <div class="col-span-12 md:col-span-6 lg:col-span-4">
                ...
                <div class="mt-4">
                    <x-form.button class="w-full">{{ __('zoker68.shop::checkout.submit') }}</x-form.button>
                </div>
            </div>
        </div>
    </form>
